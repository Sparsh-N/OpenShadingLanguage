...
# Function to find LLVM root using llvm-config
find_llvm_root() {
    if command -v llvm-config &> /dev/null; then
        LLVM_ROOT=$(dirname $(llvm-config --bindir))
        echo "LLVM_ROOT found: $LLVM_ROOT"
    else
        echo "llvm-config not found. Please install LLVM or add it to your PATH."
        exit 1
    fi
}

# Call the function to find LLVM root
find_llvm_root

# Set the LLVM root in the CMake flags
OSL_CMAKE_FLAGS="$OSL_CMAKE_FLAGS -DLLVM_ROOT=$LLVM_ROOT"
